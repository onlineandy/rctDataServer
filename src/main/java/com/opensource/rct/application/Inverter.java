...
public class Inverter {
    ...
    public void connect() {
        try {
            logger.info("<<<<< INFO >>>>> Inverter::connecting to inverter at: " + config.getHostname());
            ...
            logger.error("<<<<< ERROR >>>>> Inverter::connect Cannot connect to inverter. Check hostname " + config.getHostname() + " and port. " + config.getPort());
    ...
    boolean runSanityCheck(String key, Float value) {
        if (key.equalsIgnoreCase("DB11855B") && config.getPanelsA() != 0 && config.getPanelsB() != 0)    //DC input power panel A configured for sanity check
        {
            if (value > config.getPanelPower() * config.getPanelsA() * 1.2)    // value 20% above the theoretical value, probably wrong response from inverter
            {
        ...
        if (key.equalsIgnoreCase("0CB5D21B") && config.getPanelsB() != 0 && config.getPanelPower() != 0)    //DC input power panel B configured for sanity check
        {
            if (value > config.getPanelPower() * config.getPanelPower() * 1.2)    // value 20% above the theoretical value, probably wrong response from inverter
            {
